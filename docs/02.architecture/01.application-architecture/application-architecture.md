...
화면, API, 데이터, 보안 및 테스트 상세설계는 본 문서의 아키텍처 원칙을 기준으로 작성한다. 확정된 공통 구현 기준은 공통 설계 규칙과 결정 기록을 따르며, 남은 미결정 항목은 해당 기능 상세설계 과정에서 결정 기록으로 관리한다.
...
| 작성일 | 2026-07-16 |
| 적용 단계 | 1차 설계 |
| 설계 범위 | 시스템관리, 공통, 고객관리, 영업관리, 최소 프로젝트 기준정보, 인력관리 |
...
| 공통 | BFD-02 | 인증, 대시보드, 알림, 첨부파일, 메모 | 1차 `COM-001`; `COM-002~COM-005`와 업무 화면 내 공통 기능은 분석에서 정의한 후속 단계 적용 |
| 고객관리 | BFD-03 | 고객사, 고객담당자, 영업활동 | CUS-001~CUS-003 |
| 영업관리 | BFD-04 | 사업공고, 영업기회, 사업성 분석, 제안, 수주 | SAL-001~SAL-005 |
| 프로젝트관리 | BFD-06 | 인력투입용 프로젝트 기본정보 수동 등록·조회·수정, 기간·상태 관리 | 프로젝트 기준정보 화면 |
| 인력관리 | BFD-07 | 직원, 기술경력, 단가, 인력투입, 투입률, 협력업체, 외주인력 | RES-001, RES-003~RES-007 |

설계 범위는 위 6개 업무영역의 v1.0 기능을 의미한다. 프로젝트관리는 제품 백로그에서 확정한 최소 기준정보만 포함하고 기존 분석 문서의 계약 기반 생성 전제는 후속 정합화 대상으로 둔다. 실제 개발 순서는 메뉴구조도와 화면목록의 `1차`, `후속` 구분을 유지한다. 후속 기능도 구조적 확장 지점을 확보하되 1차 개발을 불필요하게 복잡하게 만들지 않는다.
...
현재 분석 기준의 주요 1차 화면은 다음과 같다. 사용자별 대시보드는 원천 업무기능 구현 후 지표와 구성을 확정하는 후속 범위로 둔다.

- 공통: 로그인
- 시스템관리: 사용자, 역할, 메뉴, 공통코드, 조직 관리
- 고객관리: 고객사, 고객담당자, 영업활동 관리
- 영업관리: 영업기회, 사업공고 수동 조회·등록·수정
- 프로젝트관리: 프로젝트 기준정보 등록·조회·수정, 기간·상태 관리
...
- 계약관리
- 프로젝트 일정·이슈·위험·산출물·진척률 및 독립 프로젝트 운영관리
- 예산·원가관리
- 매출·수금관리
- 유지관리
...
| 인력투입 → 프로젝트 기준정보 | 프로젝트관리 모듈이 기준정보를 소유하고 인력관리는 공개 조회 인터페이스를 통해 유효한 프로젝트를 참조한다. |
| 프로젝트 → 계약 | v1.0 수동 등록 프로젝트는 계약 없이 존재할 수 있다. v1.1 계약관리 도입 시 선택 계약 연결 또는 계약 기반 생성 기능을 추가한다. |
...
| 구분 | 현재 기준 | 설계 방향 |
| --- | --- | --- |
| 백엔드 | Java 17, Spring Boot 4.1.0, Spring MVC, MyBatis, HikariCP | REST API와 업무 트랜잭션 제공 |
...
└── infrastructure  # MyBatis 저장소, 외부 API 클라이언트 구현
```

데이터 접근은 출력 Port 또는 도메인 Repository와 MyBatis Adapter를 경계로 분리한다. API 계층과 애플리케이션 서비스는 MyBatis Mapper를 직접 호출하거나 영속 모델을 응답으로 반환하지 않는다. 세부 패키지와 SQL 기준은 [ADR-004](90.decisions/004-mybatis-data-access-standard.md)를 따른다.
...
| `project` | 프로젝트 기본정보, 기간, 연장과 상태 변경 및 인력투입용 조회계약 |
...
- 다른 업무영역의 내부 저장소, MyBatis Mapper나 영속 모델을 직접 참조하지 않는다.
...
- 프로젝트 연장은 최초종료일자를 유지하고 현재 종료일자와 상태를 같은 트랜잭션에서 변경한다.
...
- 일반 세션은 15분 유휴시간과 로그인 후 8시간 절대 유효시간을 함께 적용한다.
...
- 계정 비활성화 시 사용자별 세션을 찾지 않고 기존 세션의 다음 보호 요청에서 현재 계정 상태를 확인하여 차단한다.
...
- 역할 또는 화면 권한을 변경하면 대상 사용자의 보안버전을 증가시키고 기존 세션을 다음 보호 요청에서 종료한다. 재로그인 시 변경된 권한 스냅샷을 생성한다.
- 시스템관리자 권한 변경, 다른 사용자 비밀번호 초기화와 관리자의 계정 활성·비활성 등 중요 작업은 로그인 또는 현재 비밀번호 재인증 후 10분 안에서만 허용한다.
- 고객, 영업, 프로젝트, 인력 데이터의 행 단위 접근범위가 필요하면 조직·담당자·역할 기준을 별도 권한 규칙으로 정의한다.
...
- 업무 데이터 접근은 [ADR-004](90.decisions/004-mybatis-data-access-standard.md)의 MyBatis 계층과 SQL 작성 기준을 따른다.
- 스키마 변경은 Flyway로만 수행하고 MyBatis가 물리 스키마를 생성하거나 변경하지 않게 한다.
- JPA 의존성과 엔터티는 제거했으며, Flyway 전체 적용 및 PostgreSQL MyBatis 통합 테스트로 스키마 호환성을 검증한다.
...
| 프로젝트 기준정보 | 상태로 계획·진행·연장·완료·취소를 관리하고 인력투입이 참조 중이면 물리삭제하지 않는다. 최초종료일자는 보존하며 현재 종료일자에 연장을 반영한다. |
...
- 일반사용자를 포함한 사용자별 대시보드는 고객·영업·프로젝트·인력 등 원천 업무기능 구현 후 제공 가능한 지표와 사용자별 구성을 확정하여 후속으로 설계·구현한다.
- 업무영역별 요약 조회는 해당 영역이 제공하고 공통 대시보드는 결과를 조합한다.
- 대시보드 구현 전에는 로그인 후 현재 구현되어 있고 사용자에게 접근권한이 있는 첫 업무 메뉴로 이동하며, 접근 가능한 메뉴가 없으면 권한 안내 화면을 제공한다.
...
- 최초 적용 대상은 프로젝트, 영업기회, 사업공고, 제안과 인력이며 후속으로 계약과 계약문서를 추가한다.
- 현재 상세설계 범위에서 제외된 계약·계약문서는 해당 업무 모듈 도입 시 업무대상 생성과 권한 판정기를 함께 구현하며 업무 엔터티 없이 업무대상만 선행 생성하지 않는다.
...
| 저장소 테스트 | PostgreSQL 기반 MyBatis 매핑, 제약조건, 잠금, 조회조건과 페이징 |
...
- 인력투입과 월별 투입률 유효성
- 프로젝트 등록 시 최초종료일자와 현재 종료일자 일치, 연장 시 최초종료일자 보존 및 현재 종료일자 증가
- 완료·취소 프로젝트의 신규 인력투입 차단과 인력투입 참조 프로젝트의 물리삭제 차단
...
7. 최소 프로젝트 기준정보 화면설계, API 명세와 상태·기간 규칙 정의
8. 인력관리 화면설계, API 명세와 데이터 설계
9. CRUD Matrix 및 요구사항–BFD–화면–API–테이블 추적성 검증
10. 테스트케이스 작성
...
| ID | 항목 | 결정 | 상태 |
| --- | --- | --- | --- |
| AA-DEC-001 | 세션 저장소 | 초기 단일 인스턴스는 기본 세션, 다중 인스턴스 전환 시 공유 저장소 적용 | 확정 |
| AA-DEC-002 | 비밀번호 정책 | 12~64자, 취약·재사용 금지, 24시간 유효한 최초 등록 전용 임시 비밀번호와 조건부 변경 적용 | 확정 |
| AA-DEC-004 | 운영 파일 저장소 | 저장소 인터페이스를 적용하고 제품·보존정책은 v1.2 첨부파일 상세설계에서 결정 | 후속 |
| AA-DEC-005 | 프로젝트 참조 공급원 | `project` 모듈이 수동 등록 기준정보를 소유하고 인력관리에 조회계약 제공 | 확정 |
| AA-DEC-006 | 데이터베이스 마이그레이션 도구 | Flyway를 스키마 변경 원본으로 사용하고 PostgreSQL 통합 테스트로 검증 | 확정 |
| AA-DEC-007 | 서버 기준 시간대 | 저장·API는 UTC, 화면 표시는 Asia/Seoul | 확정 |
| AA-DEC-008 | 데이터 접근 표준 | MyBatis, HikariCP와 업무영역별 출력 Port·Repository·Adapter·Mapper 구조 적용 | 확정 |
...
| 버전 | 일자 | 변경 내용 |
| --- | --- | --- |
| 0.6 | 2026-07-24 | ADR-004 승인에 따라 MyBatis·HikariCP 기술 기준, 데이터 접근 계층, PostgreSQL 저장소 테스트와 JPA 전환 원칙 반영 |
| 0.5 | 2026-07-23 | 사용자별 대시보드를 원천 업무기능 구현 이후의 후속 범위로 전환하고 로그인 후 이동 원칙 반영 |
| 0.4 | 2026-07-21 | API 응답·시간대, 비밀번호, Flyway와 공통 설계 규칙 기준선 확정 |
| 0.3 | 2026-07-21 | v1.0 최소 프로젝트 기준정보, 수동 관리, 연장 상태와 최초·현재 종료일 관리 반영 |
