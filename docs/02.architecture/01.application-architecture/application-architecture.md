...
| 인력관리 | BFD-07 | 직원, 기술등급·기술항목, 단가, 인력투입, 투입률, 협력업체, 외주인력 | RES-001~RES-007 |
...
- 영업관리: 사업공고, 수주 관리
- 인력관리: 직원, 기술등급, 단가, 인력투입, 투입률, 협력업체, 외주인력 관리
...
- 공통 모듈은 인증, 파일, 알림, 메모 등 명확한 공통 책임만 가진다.
- 기술 편의성만을 이유로 모든 기능을 `common` 또는 `util`에 배치하지 않는다.
- 논리삭제, 사용중지, 이력 보존 정책은 데이터 성격에 따라 구분한다.
...
| `common` | 인증 유스케이스, 대시보드 조합, 알림, 첨부파일, 메모 |
...
- 외주인력 계정은 생성하지 않는다.
- 사용중지, 비활성 또는 잠금 상태의 계정은 로그인할 수 없다.
- 비밀번호는 복호화할 수 없는 단방향 해시로 저장한다.
- 로그인 실패 횟수, 계정 잠금과 초기화 정책은 시스템 상세설계에서 확정한다.
...
- 조회·등록·수정·삭제 등 행위 권한이 메뉴 접근권한만으로 부족한지는 API 상세설계 전에 결정한다.
- 고객, 영업, 인력 데이터의 행 단위 접근범위가 필요하면 조직·담당자·역할 기준을 별도 권한 규칙으로 정의한다.
- 첨부파일과 메모 권한은 연결된 업무대상의 권한을 따른다.
...
- 사용중지는 삭제 API와 구분된 상태변경 API 또는 `PATCH`로 표현한다.
...
| 데이터 유형 | 처리 원칙 |
| --- | --- |
| 사용자·역할·메뉴·조직·공통코드 | 참조 이력을 보존하기 위해 물리삭제보다 사용중지를 기본으로 한다. |
...
- 업무구분과 업무ID로 대상을 연결하며 대상 존재와 접근권한은 해당 업무영역에 위임한다.
...
- 로그인 성공·실패, 계정 잠금과 사용중지
- 역할 부여·회수 및 메뉴·API 접근 차단
- 계층형 메뉴와 조직의 순환 참조 방지
- 사용중지된 공통코드의 신규 입력 차단과 기존 데이터 조회
- 고객사–담당자–영업활동 연결
- 사업공고 중복수집 방지와 영업 전환
...
| AA-DEC-002 | 비밀번호 정책 | 단방향 해시와 실패 잠금 적용, 복잡도·변경주기는 별도 확정 | 인증 상세설계 |
| AA-DEC-003 | 역할 권한 단위 | 메뉴 접근권한 기본, 행위 권한 필요성 추가 검토 | 권한 상세설계 |
