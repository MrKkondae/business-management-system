<!-- 이 파일은 python scripts/generate_erd.py --area system 명령으로 생성합니다. 직접 수정하지 마십시오. -->
# 시스템 관리 상세 ERD
...
erDiagram
    TB_SYS_USER {
        ...
        TIMESTAMP TEMP_PWD_EXPIRE_DTM "임시비밀번호만료일시; NULL"
        INTEGER SEC_VER "보안버전; NOT NULL, DEFAULT 1"
    }
    TB_SYS_ROLE {
...
| CK_TB_SYS_USER_05 | TB_SYS_USER | PWD_INIT_REQ_YN\|PWD_CHG_DTM\|TEMP_PWD_EXPIRE_DTM | `(PWD_INIT_REQ_YN = 'Y' AND TEMP_PWD_EXPIRE_DTM IS NOT NULL AND TEMP_PWD_EXPIRE_DTM > PWD_CHG_DTM) OR (PWD_INIT_REQ_YN = 'N' AND TEMP_PWD_EXPIRE_DTM IS NULL)` | 최초 등록 필요 여부와 임시 비밀번호 만료일시 일관성 검사 |
| CK_TB_SYS_USER_06 | TB_SYS_USER | LOGIN_ID | `LOGIN_ID = LOWER(BTRIM(LOGIN_ID))` | 로그인ID 소문자 정규화 저장 검사 |
| CK_TB_SYS_USER_07 | TB_SYS_USER | SEC_VER | `SEC_VER >= 1` | 보안버전 양수 검사 |
...
```powershell
python scripts/generate_erd.py --area system
```
